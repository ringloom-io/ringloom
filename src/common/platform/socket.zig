...
//! Minimal raw socket helpers used by tooling and tests.
...
test "parse IPv4 address" {
    const addr = try Address.parseIp4("127.0.0.1", 9464);
    try std.testing.expectEqual(@as(posix.sa_family_t, posix.AF.INET), addr.any.family);
    try std.testing.expectEqual(@as(u16, 9464), std.mem.bigToNative(u16, addr.in.port));
}
