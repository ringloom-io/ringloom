...
pub const MessageFragmentingProducer = struct {
    ...
    fn sendFragment(self: *Self, msg_type: i32, chunk: []const u8, flags: u8) RingBuffer.WriteError!void {
        ...
        try self.ring_buffer.write(msg_type, chunk);
    }
};
...
test "unfragmented message sent as single record" {
    ...
    var frag_producer = MessageFragmentingProducer.init(&rb, 512);

    try frag_producer.send(1, "small message");

    var read_rb = try RingBuffer.init(buf, false, null, null);
    resetFragTestState();
    _ = read_rb.read(&fragmentCountHandler, 256);
...
test "large message is fragmented" {
    ...
    var frag_producer = MessageFragmentingProducer.init(&rb, 60);
