...
3. [Memory Layout & Shared Memory IPC](#3-memory-layout--shared-memory-ipc)
4. [Concurrent Data Structures](#4-concurrent-data-structures)
5. [Service ↔ Broker IPC](#5-service--broker-ipc)
6. [TCP Wire Protocol](#6-tcp-wire-protocol)
7. [Send Path Architecture](#7-send-path-architecture)
8. [Receive Path Architecture](#8-receive-path-architecture)
9. [Back-Pressure](#9-back-pressure)
10. [TCP Transport Library](#10-tcp-transport-library-ringloom_tcp)
11. [Threading Model](#11-threading-model)
12. [Broker Control Loop](#12-broker-control-loop)
13. [Message Routing Engine](#13-message-routing-engine)
14. [Service Registration & Discovery](#14-service-registration--discovery)
15. [Heartbeat & Health Checking](#15-heartbeat--health-checking)
16. [Cluster Management](#16-cluster-management)
17. [Configuration](#17-configuration)
18. [Counters & Monitoring](#18-counters--monitoring)
...
5. **Simplicity over generality.** This system has exactly one use case: RingLoom broker message routing. Every abstraction that doesn't serve this use case is eliminated.
...
## 3. Memory Layout & Shared Memory IPC

### 3.1 Broker Metadata File
...
### 3.3 TCP Receive Buffers

The TCP-based design requires no application-level receive buffers. TCP guarantees in-order, reliable delivery of bytes.
...
### 5.2 Same-Host Message Path (Zero Broker Involvement)
...
### 6.3 Message Frame Header (24 bytes)
...
### 7.3 Sender Event Loop Duty Cycle
...
### 8.2 TCP Framing (Length-Prefix Protocol)
...
### 8.3 Receiver Event Loop Duty Cycle
...
### 8.4 Message Routing (Receive → Service)
...
### 9.2 Why the Receiver Never Pauses
...
## 10. TCP Transport Library (`ringloom_tcp`)
...
### 11.2 Inter-Event-Loop Communication
...
Command queues use self-dispatching function pointers:
...
## 12. Broker Control Loop

### 12.1 Control Loop Duty Cycle
...
## 14. Service Registration & Discovery
...
## 15. Heartbeat & Health Checking

### 15.1 Service Heartbeat
...
### 15.3 Broker-to-Broker Heartbeat
...
### 16.1 Broker Leader Election (Bully Algorithm)
...
### 16.3 Cluster State Synchronization
...
## 18. Counters & Monitoring

### 18.1 System Counters
...
### 20.1 Required Platform Abstractions
...
### 21.5 Default Configuration Values
...
## Appendix B: End-to-End Latency Analysis

### B.1 Same-Host Path (IPC)
