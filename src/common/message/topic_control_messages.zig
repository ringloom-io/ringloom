...
//!
//! These extend the control ring-buffer message family (4-byte
//! `ControlMessageHeader{template_id, body_length}` + extern body, with a
//! variable-length name/path tail). Fields after a variable-length region use
//! `align(1)` like `ServiceInstanceEntry`.
//!
//! This module is shared by the broker and the service runtime so both encode
//! and decode the identical wire layout.

const std = @import("std");
const cm = @import("control_messages.zig");
const topic_config_mod = @import("../topics/topic_config.zig");
...
/// Ack high-water-mark feedback for `replicate_once` publishes (template 15).
///
/// `replicated_hwm` is the highest ringloom-queue master index applied by ≥1
/// replica (or the master append index on a single-node broker). `replicated_count`
/// is the monotonic per-topic count of published messages that have been
/// replicated — the service maps its per-publish sequence token against this
/// count, since the queue index (`cycle<<32 | seqnum`) resets across cycle
...
comptime {
    ...
    std.debug.assert(@sizeOf(TopicAckFeedbackMsg) == header_size + 8 + 8 + 8 + 8);
}
...
pub fn encodeAckFeedback(buf: []u8, topic_id: u64, leader_epoch: u64, replicated_hwm: u64, replicated_count: u64) usize {
...
pub fn encodeUnregisterTopicPublication(buf: []u8, local_service_id: i32, topic_id: u64) usize {
    const msg = UnregisterTopicPublicationMsg{
        .header = .{ .template_id = TEMPLATE_UNREGISTER_TOPIC_PUBLICATION, .body_length = @intCast(@sizeOf(UnregisterTopicPublicationMsg) - header_size) },
        .local_service_id = local_service_id,
        .topic_id = topic_id,
    };
    writeStruct(buf, UnregisterTopicPublicationMsg, msg);
    return @sizeOf(UnregisterTopicPublicationMsg);
}

pub fn encodeUnsubscribeTopic(buf: []u8, local_service_id: i32, topic_id: u64) usize {
    const msg = UnsubscribeTopicMsg{
        .header = .{ .template_id = TEMPLATE_UNSUBSCRIBE_TOPIC, .body_length = @intCast(@sizeOf(UnsubscribeTopicMsg) - header_size) },
        .local_service_id = local_service_id,
        .topic_id = topic_id,
    };
    writeStruct(buf, UnsubscribeTopicMsg, msg);
    return @sizeOf(UnsubscribeTopicMsg);
}
...
test "publication response + ack feedback fixed-size encode" {
    ...
    const n2 = encodeAckFeedback(&buf, 7, 5, 4096, 128);
    const a = decode(TopicAckFeedbackMsg, buf[0..n2]).?;
    try testing.expectEqual(@as(u64, 4096), a.replicated_hwm);
    try testing.expectEqual(@as(u64, 128), a.replicated_count);
}

test "unregister + unsubscribe round-trip" {
    var buf: [64]u8 = undefined;
    const n = encodeUnregisterTopicPublication(&buf, 42, 1234);
    const m = decode(UnregisterTopicPublicationMsg, buf[0..n]).?;
    try testing.expectEqual(@as(i32, 42), m.local_service_id);
    try testing.expectEqual(@as(u64, 1234), m.topic_id);

    const n2 = encodeUnsubscribeTopic(&buf, 42, 1234);
    const u = decode(UnsubscribeTopicMsg, buf[0..n2]).?;
    try testing.expectEqual(@as(i32, 42), u.local_service_id);
    try testing.expectEqual(@as(u64, 1234), u.topic_id);

    const n3 = encodeTopicLeaderChanged(&buf, 1234, 2, 9);
    const lc = decode(TopicLeaderChangedMsg, buf[0..n3]).?;
    try testing.expectEqual(@as(u64, 1234), lc.topic_id);
    try testing.expectEqual(@as(i16, 2), lc.leader_node_id);
    try testing.expectEqual(@as(u64, 9), lc.leader_epoch);
}
